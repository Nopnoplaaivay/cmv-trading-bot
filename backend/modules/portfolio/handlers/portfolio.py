...
async def get_portfolio_by_id(portfolio_id: str, user: JwtPayload = Depends(UserPayload)):
    portfolio = await PortfoliosService.get_portfolio_by_id(portfolio_id=portfolio_id, user=user)
    if not portfolio:
        response = BaseResponse(
            http_code=404,
            status_code=404,
...
async def create_custom_portfolio(
    payload: CreateCustomPortfolioDTO,
